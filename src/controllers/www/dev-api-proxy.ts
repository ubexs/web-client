...
export class DevApiProxy extends base {
    ...
    public async allMethods(
        ...
        if ((result.headers['content-type'] as string).match(/json/g) || (result.headers['content-type'] as string).match(/javascript/g) || (result.headers['content-type'] as string).match(/text/g)) {
            res.set(headers);
            res.status(result.status);
